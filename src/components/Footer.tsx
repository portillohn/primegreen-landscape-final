...
export default function Footer() {
  return (
    <footer className="bg-[#152C17] text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-16 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="text-green-400 text-sm font-medium mb-3 italic">
            ...
            <div className="space-y-2 mb-6">
              {trustItems.map(({ icon: Icon, label }) => (
                <div key={label} className="flex items-center gap-2 text-xs text-green-300/80">
            ...
            <div className="flex gap-3">
              <a href="https://www.facebook.com/PrimeGreenLandscape/" target="_blank" rel="noopener noreferrer"
                aria-label="Facebook" className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-green-300 hover:bg-green-700 hover:text-white transition-all">
              ...
              <a href="https://www.instagram.com/primegreenmoco/" target="_blank" rel="noopener noreferrer"
                aria-label="Instagram" className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-green-300 hover:bg-green-700 hover:text-white transition-all">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          {/* Col 2 — Quick Links */}
          <div>
            <p className="text-xs font-black uppercase tracking-widest text-green-400 mb-4">Navigation</p>
            <ul className="space-y-2.5">
              {quickLinks.map(link => (
                <li key={link.label}>
                  <Link href={link.href} className="text-sm text-green-200/70 hover:text-white transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          {/* Col 3 — Services */}
          <div>
            <p className="text-xs font-black uppercase tracking-widest text-green-400 mb-4">Our Services</p>
            <ul className="space-y-2.5">
              {services.map(s => (
                <li key={s.label}>
                  <Link href={s.href} className="text-sm text-green-200/70 hover:text-white transition-colors">
                    {s.label}
                  </Link>
                </li>
              ))}
            ...
            <div className="mt-5 pt-5 border-t border-white/10">
              <p className="text-xs font-black uppercase tracking-widest text-green-400 mb-3">Service Areas</p>
              <ul className="grid grid-cols-2 gap-x-2 gap-y-2">
                {cities.map((city) => (
                  <li key={city.slug}>
                    <Link href={`/areas/${city.slug}`}
                      className="text-xs text-green-200/70 hover:text-white transition-colors flex items-center gap-1">
          ...
          {/* Col 4 — Contact / CTA */}
          <div>
            <p className="text-xs font-black uppercase tracking-widest text-green-400 mb-4">Get In Touch</p>

            <div className="space-y-3 mb-6">
              <div className="flex items-start gap-2.5">
                <Mail className="w-3.5 h-3.5 text-green-400 mt-0.5 flex-shrink-0" />
                <a href="mailto:kevin@primegreenlandscape.com"
                  className="text-sm text-green-200/70 hover:text-white transition-colors break-all">
              ...
              <div className="flex items-start gap-2.5">
                <Clock className="w-3.5 h-3.5 text-green-400 mt-0.5 flex-shrink-0" />
                <span className="text-sm text-green-200/70">
            ...
            <div className="bg-white/5 rounded-xl p-4 border border-white/10 mb-5">
              ...
              <p className="text-xs text-green-200/70 leading-snug">
                Trusted by homeowners across Montgomery County.
              </p>
            </div>

            <Link href="/contact"
              className="block w-full text-center py-3 bg-green-600 hover:bg-green-500 text-white font-bold text-sm rounded-xl transition-all shadow-lg">
              Get a Free Quote
            </Link>
          </div>
